...
//
// 1. Создайте переменную `sum` целочисленного типа и присвойте ей значение 150
// 2. Прибавьте к ней 100 и сохраните результат сложения обратно в переменную `sum`
// 3. Используйте короткую запись сложения
//
...
TestRunner.runTests(.default(sum))
